/* Nousomplex Attendance Portal — Optimized for faster loading */
(() => {
  ...
  function showLoadingScreen(message = 'Loading...') {
    const loadingScreen = document.getElementById('loading-screen');
    if (loadingScreen) {
      const subText = loadingScreen.querySelector('.sub-text');
  ...
  // Cache classes to avoid repeated API calls
  ...
  function classOptions(selected = "", none = "Select a class") { 
    return `<option value="">${none}</option>` + state.classes.map(c => `<option value="${c.id}" ${c.id === selected ? "selected" : ""}>${esc(c.name)}${c.section ? " — " + esc(c.section) : ""}</option>`).join(""); 
  }
  ...
  async function loadSession() {
    try {
      ...
      // Load profile and teacher data in parallel for speed
  ...
  // --- Dashboard with optimized data loading ---
  ...
  // --- Rest of the functions (attendance, students, classes, teachers, reports) ---
  // These remain the same as before, just make sure getClasses() is used with caching
  ...
  async function loadRoster() {
    const classId = $("#attendance-class").value, date = $("#attendance-date").value; 
  ...
  async function reports() {
    ...
    $("#report-class").innerHTML = classOptions("", "All available classes");
  ...
  function init() {
    ...
    $("#signout").onclick = async () => { 
      ...
      cachedClasses = null; // Clear cache on sign out
      showAuth(); 
    };
